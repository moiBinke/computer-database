...
//																	  .addValue("taillePage", taillePage);
	...
	public ArrayList<Computer> search(String search) {
		...
		Predicate byName=criteriaBuilder.like(root.get("name"), search);
		Predicate byCompanyName=criteriaBuilder.like(root.get("company.name"), search);
		Predicate orSearch=criteriaBuilder.or(byName,byCompanyName);
		criteriaQuery.where(byName);
